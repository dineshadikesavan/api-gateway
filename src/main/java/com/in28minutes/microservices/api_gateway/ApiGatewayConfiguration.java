...
public class ApiGatewayConfiguration {
    ...
    public RouteLocator gatewayRouter(RouteLocatorBuilder builder) {
        return builder.routes()
                .route(rf -> rf.path("/get")
                        .filters(f -> f.addRequestHeader("MyHeader", "MyURI")
                                .addRequestParameter("MyParam", "MyParamValue")
                            )
                        .uri("http://httpbin.org:80")
                )
                .route(rf -> rf.path("/currency-exchange/**")
                        .uri("lb://currency-exchange")
                )
                .route(rf -> rf.path("/currency-conversion/**")
                        .uri("lb://currency-conversion")
                )
                .route(rf -> rf.path("/currency-conversion-feign/**")
                        .uri("lb://currency-conversion")
                )
                .route(rf -> rf.path("/currency-conversion-new/**")
